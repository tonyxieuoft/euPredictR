# Purpose: Display Gene Predictions as Phylogenies
# Author: Tony Xie
# Date: 2024-11-27
# Version: 0.1.0
...
#' along with accompaning plot data.
...
display_phylogeny <- function(predictions, gene_name){
  ...
  phylo_plot <- suppressMessages(msa::msa(dna_set, "ClustalW")) %>%
    msa::msaConvert("phangorn::phyDat") %>%   # convert to phyDat format
    phangorn::dist.ml() %>%                   # construct distance matrix
